...
const StudentRegister = ({ registeredIds = [], setRegisteredIds }) => {
  ...
  const registerCourse = (course) => {
    ...
    if (registeredIds.includes(course.id))
      return toast.error("Bạn đã đăng ký môn này.");
    if (course.registered >= course.capacity)
      return toast.error("Lớp học phần đã hết chỗ trống.");
    if (totalCredits + course.credits > MAX_CREDITS)
      return toast.error(`Không được vượt quá ${MAX_CREDITS} tín chỉ.`);
    if (hasTimeConflict(course, registeredCourses))
      return toast.error("Không được đăng ký trùng thời gian học.");
        ...
        >
          ...
          <tbody>
            {filteredCourses.map((course, index) => {
              ...
              const remaining =
                course.capacity - course.registered - (isRegistered ? 1 : 0);
              return (
