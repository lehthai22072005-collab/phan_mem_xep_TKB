...
const getSemesterErrorMessage = (err, action = "save") => {
  ...
  if (lowerMessage.includes("unique") || lowerMessage.includes("duplicate") || lowerMessage.includes("p2002") || lowerMessage.includes("name_school_year") || lowerMessage.includes("semester_name_school_year_key")) {
    return "Kỳ học này đã tồn tại trong năm học đã chọn. Vui lòng kiểm tra lại tên kỳ học và năm học.";
  }
  if (lowerMessage.includes("start_date must be before end_date") || lowerMessage.includes("start date") || lowerMessage.includes("before end date")) {
    return "Ngày bắt đầu phải trước hoặc bằng ngày kết thúc.";
  }
  if (lowerMessage.includes("start_date and end_date are required") || lowerMessage.includes("required")) {
    return "Vui lòng nhập đầy đủ ngày bắt đầu và ngày kết thúc.";
  }
  if (lowerMessage.includes("cannot update semester date range") || lowerMessage.includes("outside the new semester date range")) {
  ...
  if (lowerMessage.includes("cannot delete semester that has courses") || lowerMessage.includes("has courses")) {
...
const MinistrySemesters = () => {
  ...
  const handleDelete = async semester => {
    if (!window.confirm(`Bạn có chắc muốn xóa kỳ học ${semester.name} ${semester.school_year}?`)) {
      return;
    }
    try {
  ...
  return <div className="ministry-semesters__page">
      <div className="ministry-semesters__header">
        <div>
          <div className="ministry-semesters__breadcrumb">Dashboard / Kỳ học</div>
          <h1 className="ministry-semesters__title">Quản lý kỳ học</h1>
        </div>
        <button type="button" onClick={handleOpenCreate} className="ministry-semesters__add-btn">
          <FiPlus size={16} />
          Thêm kỳ học mới
        </button>
      </div>

      {showForm && <div className="ministry-semesters__modal-overlay">
          <div className="ministry-semesters__modal-card">
            <div className="ministry-semesters__modal-header">
              ...
              <button type="button" onClick={closeForm} className="ministry-semesters__close-btn">
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="ministry-semesters__form-grid">
                <label className="ministry-semesters__field">
                  Tên kỳ học
                  <input name="name" value={formData.name} onChange={handleChange} placeholder="HK1, HK2, HK Hè" required className="ministry-semesters__input" />
                
                </label>
                <label className="ministry-semesters__field">
                  Năm học
                  <input name="school_year" value={formData.school_year} onChange={handleChange} placeholder="2025-2026" required className="ministry-semesters__input" />
                
                </label>
                <label className="ministry-semesters__field">
                  Ngày bắt đầu
                  <input type="date" name="start_date" value={formData.start_date} onChange={handleChange} required className="ministry-semesters__input" />
                
                </label>
                <label className="ministry-semesters__field">
                  Ngày kết thúc
                  <input type="date" name="end_date" value={formData.end_date} onChange={handleChange} required className="ministry-semesters__input" />
              ...
              <div className="ministry-semesters__modal-footer">
                <button type="button" onClick={closeForm} className="ministry-semesters__cancel-btn">
                  Hủy
                </button>
      ...
      <div className="ministry-semesters__table-card">
        ...
        <div className="ministry-semesters__inline-292">
          <table className="ministry-semesters__table">
            ...
            <tbody>
              {semesters.map(semester => <tr key={semester.semester_id} className="ministry-semesters__tr">
                  <td className="ministry-semesters__td">{semester.name}</td>
                  <td className="ministry-semesters__td">{semester.school_year}</td>
                  ...
                  <td className="ministry-semesters__td">
                    <span className={semester.is_active ? "ministry-semesters__active-badge" : "ministry-semesters__badge"}>
                      {semester.is_active ? "Hiện hành" : "Chưa kích hoạt"}
                    </span>
                  </td>
                  <td className="ministry-semesters__td">
                    {!semester.is_active && <button type="button" onClick={() => handleActivate(semester.semester_id)} className="ministry-semesters__activate-btn">
                    
                        Đặt hiện hành
                      </button>}
                    <button type="button" onClick={() => handleEdit(semester)} className="ministry-semesters__edit-btn">
                    
                      <FiEdit2 size={13} /> Sửa
                    </button>
                    <button type="button" onClick={() => handleDelete(semester)} className="ministry-semesters__delete-btn">
              ...
              {semesters.length === 0 && <tr>
                  <td colSpan={6} className="ministry-semesters__empty">
                    Chưa có kỳ học nào
                  </td>
                </tr>}
            </tbody>
